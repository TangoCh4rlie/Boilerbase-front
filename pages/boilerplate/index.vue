...
<template>
    <h3>New Boilerplate</h3>
    <UForm :schema="newBoilerplateValidator" :state="boilerplate" @submit="onSubmit">
        <UFormGroup label="Git URL" name="gitUrl" required>
            <UInputMenu v-model="boilerplate.projectName" :options="names"/>
        </UFormGroup>
        <UFormGroup label="Boilerplate name" name="boilerplate_name" required>
        ...
        <UFormGroup label="Language" name="language" required>
            <UBadge v-for="(item, index) in languages" :key="index" :label="item.label" :color="item.color"
                    @click="languages.pop()"/>
